...
Revenue1 <- rep("",length(data$revenue))
Revenue1[data$Revenue== TRUE]    <- "Yes"
Revenue1[data$Revenue== FALSE]    <- "No"
plot(factor(Revenue1)~factor(VisitorType), data=data,
...
plot(factor(Revenue1)~factor(SpecialDay), data=data,
...
b <- ggplot(data,aes(x=as.factor(Revenue1),y=ProductRelated_Duration))+
  geom_boxplot()+
  labs(x='Purchase or Not',
...
c <- ggplot(data,aes(x=as.factor(Revenue1),y=Informational_Duration))+
  geom_boxplot()+
  labs(x='Purchase or Not',
...
d <- ggplot(data,aes(x=as.factor(Revenue1),y=Informational_Duration))+
  geom_boxplot()+
  labs(x='Purchase or Not',
...
newdata.min.holdout <- data.frame(Mx_holdout[,features.min],My_holdout)
newdata.1se.holdout <- data.frame(Mx_holdout[,features.1se],My_holdout)
newdata.theory.holdout <- data.frame(Mx_holdout[,features.theory],My_holdout)
...
barplot(colMeans(holdout.F1), las=2,xpd=FALSE, xlab="", ylab = "F1")
names(OOS)[8] <-"logistic.interaction"
names(OOS.ACC)[8] <-"logistic.interaction"
names(OOS.TPR)[8] <-"logistic.interaction"
names(OOS.FPR)[8] <-"logistic.interaction"
...
print(paste('units=',units*50,'drop=',drop*0.04,'values=',values$F1))
F1$drop[drop+units*10]=drop*0.04
F1$units[drop+units*10]=units*50
F1$regular[drop+units*10]=0.06
F1$F1[drop+units*10]=values$F1
